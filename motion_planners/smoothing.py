...
from motion_planners.utils import INF, elapsed_time, irange, waypoints_from_path, pairs, get_distance, \
    convex_combination, flatten, randomize

import time
import numpy as np

def smooth_path_old(path, extend, collision, iterations=50, max_time=INF):
    ...
    for _ in irange(iterations):
        ...
        if (len(shortcut) < (j - i)) and all(not collision(q) for q in randomize(shortcut)):
            smoothed_path = smoothed_path[:i + 1] + shortcut + smoothed_path[j + 1:]
    return smoothed_path

def smooth_path(path, extend, collision, distance_fn=None, iterations=50, max_time=INF):
    ...
    for _ in irange(iterations):
        ...
        if all(not collision(q) for q in randomize(extend(point1, point2))):
            i, _ = segment1
            _, j = segment2
